...
void RobStrideInit(RobStride_t *device, FDCAN_HandleTypeDef *hfdcan, uint32_t id, RobStrideMode mode, RobStrideType type)
{
 ...
 uint32_t RobStrideSend(RobStride_t *device, uint32_t ExtID, uint8_t *buf)
{
    ...
    TxHeader.DataLength = FDCAN_DLC_BYTES_8;  // 发送8字节数据
    ...
    HAL_StatusTypeDef status = HAL_FDCAN_AddMessageToTxFifoQ(device->hfdcan, &TxHeader, buf);
    if (status != HAL_OK) {
        return 1; // 发送失败
    }
    return 0; // 发送成功
